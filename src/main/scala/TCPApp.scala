...
/**
  * The <a href="https://github.com/typelevel/fs2-chat">fs2-chat</a> sample application implements a
  * multiuser chat server and a single user chat client using the FS2 TCP support and <a
  * href="https://scodec.org/">scodec</a> for binary processing.
  */
object TCPApp extends IOApp.Simple {
  ...
  def connect[F[_]: Temporal: Network](address: SocketAddress[Host]): Stream[F, Socket[F]] =
    socketStream(address).handleErrorWith { case _: ConnectException =>
  ...
  def echoServer[F[_]: Concurrent: Network]: F[Unit] = Network[F]

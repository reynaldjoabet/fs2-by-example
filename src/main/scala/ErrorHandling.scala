...
object ErrorHandling extends IOApp.Simple {
  ...
  override def run: IO[Unit] = {
    ...
    val s3 =
      Stream.repeatEval(IO.println("emitting").as(42)).take(3) ++ Stream
        .raiseError[IO](new Exception("error after"))
    val s4 = Stream.raiseError[IO](new Exception("error before")) ++ Stream
      .eval(IO.println("the end!"))
